...
if ($_SESSION['user_group_id']=='1') {
  $bookinghistory = "SELECT * FROM carpool_history WHERE driver_id='$_SESSION[user_id]'";
...
<head>
  ...
  <title>Driver History - Car Pool System</title>
...
<section class="header9 cid-ruv4FB75N7 mbr-fullscreen mbr-parallax-background" id="header9-1">
    ...
    <div style="background-color:rgba(0, 0, 0, 0.5); margin-top:4%;" class="container">
        <div  class="media-container-column mbr-white col-lg-8 col-md-10">
          <h4 style="color:white;" class="align-left mbr-bold pt-5 pb-3 mbr-fonts-style display-6">Welcome <?php echo $_SESSION['fname']; ?>,<br><br>Driving History Dashboard</h4>
          <div style="width:100%;" class="">
            <table  class="table isSearch" cellspacing="0">
              <thead>
                <tr class="table-heads">
                  <th class="head-item mbr-fonts-style display-7">Booking ID</th>
                  <th class="head-item mbr-fonts-style display-7">Client's Name</th>
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td class="body-item mbr-fonts-style display-7"><?php  $userresult = $conn->query($userdetails); while($rowuser=$userresult->fetch_assoc()){ if($rowhistory['user_id']==$rowuser['user_id']){echo $rowuser['firstname'].' '.$rowuser['lastname'];}} ?></td>
                  <td class="body-item mbr-fonts-style display-7"><?php $userresulta = $conn->query($userdetails); while($rowuser=$userresulta->fetch_assoc()){if($rowhistory['user_id']==$rowuser['user_id']){echo $rowuser['community'];} }?></td>
                  <td class="body-item mbr-fonts-style display-7"><?php echo $rowhistory['date']; ?></td>
                  <td class="body-item mbr-fonts-style display-7"><?php   $userresultb = $conn->query($userdetails); while($rowuser=$userresultb->fetch_assoc()){ if($rowhistory['user_id']==$rowuser['user_id']){   $communityresult = $conn->query($communitysql); while($rowcomm = $communityresult->fetch_assoc()){if($rowcomm['community_name']==$rowuser['community']){echo $rowcomm['community_price'];}}}} ?></td>
                </tr>
              <?php } ?>
              </tbody>
            </table>
